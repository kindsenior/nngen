...
def MatMul(visitor, node,
    ...
    # <<< needless ?
    orig_shape = input.get_original_shape() # 2 dim
    orig_layout = input.get_original_layout() # 4 dim
    orig_onnx_layout = input.get_original_onnx_layout() # 4 dim
    # orig_layout = visitor.nngen_input_layout
    # orig_onnx_layout = visitor.onnx_input_layout
    ...
    # for transposed_b = True
    new_value = filter.value.transpose()
    filter.shape = new_value.shape
    filter.value = new_value
    ...
    #rshift_out_name = '_'.join(['onnx', name, 'matmul.rshift_out'])
